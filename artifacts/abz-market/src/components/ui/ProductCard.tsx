...
export function ProductCard({ product }: ProductCardProps) {
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    toast({ title: "Savatga qo'shildi", description: `${product.name} savatga tushdi.`, duration: 2000 });
  };
      ...
      <div className="bg-card border border-border/50 rounded-2xl overflow-hidden h-full flex flex-col hover:shadow-lg hover:border-primary/20 transition-all duration-300 active:scale-[0.98]">
        <div className="relative aspect-square overflow-hidden bg-muted/30">
          ...
          <button
            ...
            className={cn(
              "absolute top-2 right-2 p-1.5 rounded-full backdrop-blur-md transition-all shadow-sm active:scale-90",
              fav
                ? "bg-red-500 text-white"
                : "bg-background/50 text-foreground/70 hover:text-destructive hover:bg-background"
            )}
          >
